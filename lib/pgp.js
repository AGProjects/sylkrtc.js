...
class PGP extends EventEmitter {
    ...
    decryptFile(fileData, filename, filetype) {
        ...
        return new Promise((resolve, reject) => {
            ...
            channel.port1.onmessage = function({ data }) {
                if (data.error !== undefined) {
                    ...
                    const raw = data.original !== undefined ? data.original : fileData;
                    resolve({ file: new File([raw], filename, { 'type': filetype }), didDecrypt: false });
                } else {
            ...
            worker.postMessage({ action, inputData, msg }, transfer);
        });
    }
    ...
    async signDetached(text) {
        ...
        return openpgp.sign({
            message,
            signingKeys: this._privateKey,
            detached: true,
            format: 'armored'
        });
    }

    async verifyDetached(armoredSignature, text, uri) {
        let publicKey;
        try {
            const armoredKey = await this._lookupPublicKey(uri);
            if (!armoredKey) {
                return { verified: null, hadKey: false };
            }
            publicKey = await openpgp.readKey({ armoredKey });
        } catch (error) {
            DEBUG('Could not look up public key for %s: %s', uri, error);
            return { verified: null, hadKey: false };
        }
        ...
        try {
            ...
            return { verified: true, hadKey: true };
        } catch (error) {
            DEBUG('Signature verification failed for %s: %s', uri, error);
            return { verified: false, hadKey: true };
        }
    }
