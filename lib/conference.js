...
class Participant extends EventEmitter {
    ...
    setAudioLevel(level) {
        ...
        this._audioLevel = Number(level) || 0;
        this.emit('audioLevelChanged', lvl);
    }
    ...
    _handleOffer(offerSdp) {
        ...
        const mungedSdp = utils.mungeSdp(offerSdp, true);
...
class ConferenceCall extends EventEmitter {
    ...
    _handleEvent(message) {
        if (message.event !== 'conference-audio-levels') {
        ...
        switch (message.event) {
            ...
            case 'initial-publishers':
                ...
                for (let p of message.publishers) {
                    participant = new Participant(p.id, new utils.Identity(p.uri, p.display_name), this, p);
            ...
            case 'publishers-joined':
                for (let p of message.publishers) {
                    ...
                    if (this._participants.get(p.id)) {
                        DEBUG('Participant already joined, skipping: %o', p);
                        continue;
                    }
                    participant = new Participant(p.id, new utils.Identity(p.uri, p.display_name), this, p);
                    this._participants.set(participant.id, participant);
            ...
            case 'conference-audio-levels':
                for (const level of (message.levels || [])) {
                    participant = this._participants.get(level.participant_id);
                    if (participant) {
                        participant.setAudioLevel = level.rx_peak ?? level.rx ?? 0;
                    }
                }
                break;
            default:
