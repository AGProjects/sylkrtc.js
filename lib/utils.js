...
function removeAllowExtmapMixed() {
...
export function serializeMessageMetadata(metadata) {
...
export function comparePGPKeys(key1, key2) {
    return Promise.all([
        readKey({ armoredKey: key1 }),
        readKey({ armoredKey: key2 })
    ])
        .then(([publicKey1, publicKey2]) => {
            return publicKey1.hasSameFingerprintAs(publicKey2);
        })
        .catch(error => {
            return false;
        });
