...
class ZrtpSession extends EventEmitter {
    ...
    installSender(sender) {
        ...
        // Video added after the ZRTP handshake for this call already
        // started (state !== 'idle') is a mid-call upgrade, not a video
        // call from the start. Mobile doesn't support encrypted video
        // added this way - skip codec resolution entirely and leave it
        // in permanent passthrough rather than attempting (and failing)
        // to activate encryption for it.
        if (kind === 'video' && this._state !== 'idle') {
            DEBUG('Video sender added after ZRTP already started (state=%s) - skipping E2E for upgraded video', this._state);
            return;
        }
    ...
    installReceiver(receiver) {
        ...
        if (kind === 'video' && this._state !== 'idle') {
            DEBUG('Video receiver added after ZRTP already started (state=%s) - skipping E2E for upgraded video', this._state);
            return;
        }
