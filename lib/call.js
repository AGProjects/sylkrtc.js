...
import { ZrtpSession, ENCRYPTION_MODES } from './zrtp';
...
class Call extends EventEmitter {
    constructor(account) {
        ...
        this._zrtp    = new ZrtpSession(this);
    ...
    answer(options = {}) {
        ...
        for (const sender of this._pc.getSenders()) {
            this._zrtp.installSender(sender);
        }
    ...
    _initOutgoing(uri, options={}) {
        ...
        this._encryptionMode = ENCRYPTION_MODES.includes(options.encryptionMode)
            ? options.encryptionMode
            : this._account.encryptionMode;
    ...
    _initIncoming(id, caller, sdp, callId, headers = {}) {
        ...
        this._encryptionMode = this._account.encryptionMode;
            ...
            case 'message':
                ...
                if (!incomingMessage) {
                    ...
                    if (ScreenSharingManager.handles(message.content_type)) {
                     ...
                     if (ZrtpSession.handles(message.content_type)) {
                        if (this.zrtpEnabled) {
                            this._zrtp.handleSignal(message.content);
                        }
                        return;
                    }
    ...
    _initRTCPeerConnection(pcConfig) {
        ...
        this._pc = new RTCPeerConnection({...pcConfig, encodedInsertableStreams: this.zrtpEnabled});
        ...
        this._pc.addEventListener('track', (event) => {
            if (this.zrtpEnabled) {
                this._zrtp.installReceiver(event.receiver);
            }
        });
