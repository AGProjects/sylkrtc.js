...
class Account extends EventEmitter {
    ...
    decryptFile(file, filename) {
        ...
        return this._pgp.decryptFile(file, filename);
    }
                    ...
                    switch (message.action) {
                        ...
                        case 'add':
                            ...
                            (async () => {
                                ...
                                if (content.content_type === 'application/sylk-file-transfer') {
                                    let parsed = JSON.parse(content.content);
                                    if (parsed.filename.endsWith('.asc')) {
                                        content.isSecure = true;
                                    }
                                }
                                const outgoingMessage = new Message(
                                    ...
                                    content.account == this._id ? 'accepted' : 'pending'
                                );
