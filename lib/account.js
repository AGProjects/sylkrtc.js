    ...
    checkIfKeyExists(cb=null) {
        ...
        new Promise((resolve, reject) => {
            this._connection.once('publicKey', (message) => {
                resolve(message.publicKey.trim());
            });
    ...
    _handleEvent(message) {
        ...
        switch (message.event) {
            ...
            case 'sync-conversations':
                ...
                (async() => {
                    if (this._pgp !== null) {
                        const promise = await message.messages.map(message => {
                            if (message.content.includes(`-----BEGIN PGP MESSAGE-----`)
                                && message.content.trim().endsWith(`-----END PGP MESSAGE-----`)
                            ) {
                                return this._pgp.decryptMessage(message);
                            }
                            return message;
                        });
                    ...
                    const messageList = message.messages.filter((message, idx) => {
                        ...
                        if (results.length !== 0 && results[idx].didDecrypt === false) {
                            return false;
                        }
                        return true;
                    }).map((message, idx) => {
                        ...
                        if (message.direction === "outgoing") {
                            ...
                            if (results.length !== 0 && results[idx].didDecrypt) {
                                message._isSecure = true;
                            }
